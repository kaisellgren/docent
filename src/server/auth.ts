...
function google(): Google {
  ...
  return new Google(
    ...
    `${configuration.APP_URL}/auth/google/callback`,
  )
}
...
export async function currentSession(): Promise<Session | undefined> {
  ...
  try {
    ...
    if (!avatarUrl) {
      try {
        const user = await (
          ...
          sql.type(
            z.object({ avatarUrl: z.string().nullable() }),
          )`SELECT avatar_url AS "avatarUrl" FROM app_user WHERE id = ${payload.userId}`,
        )
    ...
    return {
      ...
      name: payload.name,
      avatarUrl,
      isEditor: editorEmails().has(payload.email.toLowerCase()),
    }
  } catch {
    return undefined
  }
...
export async function completeGoogleSignIn(code: string, state: string): Promise<void> {
  ...
  const session: Session = {
    ...
    isEditor: editorEmails().has(profile.email.toLowerCase()),
  }
