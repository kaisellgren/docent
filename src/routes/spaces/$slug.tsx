...
function WikiPage() {
  ...
  async function erase() { if (!window.confirm(`Soft-delete “${page.title}”?`)) return; await run('delete this page', async () => { await remove({ data: { slug: page.slug } }); await router.navigate({ to: '/spaces' }); }); }
