...
export const Route = createFileRoute('/spaces/new')({
  validateSearch: (search: Record<string, unknown>) => ({ spaceId: typeof search.spaceId === 'string' ? search.spaceId : '' }),
  ...
  loader: async ({ deps }) => {
    ...
    return { viewer, spaces, initialPages: initialSpaceId ? await getSpacePages({ data: { spaceId: initialSpaceId } }) : [], initialSpaceId };
  },
...
function CreatePage() {
  ...
  useEffect(() => { setSpaceId(initialSpaceId); setParentPageId(''); setParentPages(initialPages); }, [initialPages, initialSpaceId]);
  useEffect(() => {
    setParentPageId('');
    if (!spaceId) { setParentPages([]); return; }
    if (spaceId === initialSpaceId) { setParentPages(initialPages); return; }
  ...
  async function publish(event: FormEvent) {
    ...
    try {
      if (!spaceId) { setError('Create a space before publishing a page.'); return; }
      const page = await create({ data: { title, markdown, spaceId, parentPageId: parentPageId || null } });
  ...
  if (!viewer) return <div className={styles.shell}><header className={styles.nav}><Link className={styles.link} to="/spaces">← Spaces</Link></header><section className={styles.section}><h1>Create page</h1><a className={styles.primaryButton} href="/auth/google">Sign in to create a page</a></section></div>;
  if (!viewer.isEditor) return <div className={styles.shell}><header className={styles.nav}><Link className={styles.link} to="/spaces">← Spaces</Link></header><section className={styles.section}><h1>Create page</h1><p className={styles.muted}>Only editors can create pages.</p></section></div>;

  return <><header className={styles.editorNav}><div className={styles.editorNavInner}><Link className={styles.editorBrand} to="/"><DocentMark />Docent</Link><div className={styles.editorCrumb}>Spaces <span className={styles.editorCrumbSeparator}>/</span> <strong className={styles.editorCrumbCurrent}>{title || 'Untitled page'}</strong></div><div className={styles.editorNavRight}><span className={styles.statusPill}><i className={styles.statusPillDot} />New draft</span><Link className={styles.editorButton} to="/spaces">Cancel</Link><button className={styles.editorPrimaryButton} form="create-page" disabled={publishing || !spaceId}>{publishing ? 'Publishing…' : 'Publish'}</button></div></div></header><main className={styles.shell}><form id="create-page" className={styles.editorShell} onSubmit={publish}><section><input className={styles.editorTitleInput} value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Untitled page" disabled={publishing} required autoFocus /><div className={styles.editorMeta}><span className={styles.editorSpace}>⌁ {spaces.find((space) => space.id === spaceId)?.name ?? 'Choose a space'}</span><span>Draft</span><span>·</span><span>Editing as {viewer.name}</span></div><div className={styles.editorToolbar}><div className={styles.formatGroup}><button type="button" className={styles.formatButton} onClick={() => insert('**', '**')} title="Bold"><b>B</b></button><button type="button" className={styles.formatButton} onClick={() => insert('_', '_')} title="Italic"><i>i</i></button><button type="button" className={styles.formatButton} onClick={() => insert('## ')} title="Heading">H</button><span className={styles.formatDivider} /><button type="button" className={styles.formatButton} onClick={() => insert('- ')} title="Bulleted list">≡</button><button type="button" className={styles.formatButton} onClick={() => insert('[', '](https://)')} title="Link">⛓</button><button type="button" className={styles.formatButton} onClick={() => insert('`', '`')} title="Code">&lt;/&gt;</button><button type="button" className={styles.formatButton} onClick={() => insert('> ')} title="Quote">“</button></div><div className={styles.viewSwitch}>{(['split', 'write', 'preview'] as const).map((view) => <button type="button" className={mode === view ? styles.viewSwitchActive : styles.viewSwitchButton} key={view} onClick={() => setMode(view)}>{view.charAt(0).toUpperCase() + view.slice(1)}</button>)}</div></div><div className={`${styles.editorPanes} ${mode === 'write' ? styles.editorPanesWrite : mode === 'preview' ? styles.editorPanesPreview : ''}`}><div className={`${styles.editorPane} ${styles.editorWritePane}`}><div className={styles.editorPaneLabel}>Markdown body</div><textarea ref={editor} className={styles.editorTextarea} value={markdown} onChange={(event) => setMarkdown(event.target.value)} placeholder="Start writing in Markdown…" disabled={publishing} spellCheck={false} required /></div><div className={`${styles.editorPane} ${styles.editorPreviewPane}`}><div className={styles.editorPaneLabel}>Preview</div><div className={styles.editorPreview}>{markdown ? <ReactMarkdown>{markdown}</ReactMarkdown> : <p>Markdown preview will appear here.</p>}</div></div></div>{error && <p className={styles.feedbackError} role="alert">{error}</p>}</section><aside className={styles.editorSidebar}><div className={styles.editorSideCard}><h2 className={styles.editorSideCardTitle}>Page settings</h2><div className={styles.editorField}><label htmlFor="space">Space</label><select id="space" className={styles.editorSelect} value={spaceId} onChange={(event) => setSpaceId(event.target.value)} disabled={publishing}><option value="">Choose a space</option>{spaces.map((space) => <option key={space.id} value={space.id}>{space.name}</option>)}</select>{spaces.length === 0 && <Link className={styles.editorCreateSpaceLink} to="/spaces">Create a space first</Link>}</div><div className={styles.editorField}><label htmlFor="parent-page">Parent page</label><select id="parent-page" className={styles.editorSelect} value={parentPageId} onChange={(event) => setParentPageId(event.target.value)} disabled={publishing || !spaceId}><option value="">None (top level)</option>{parentPages.map((page) => <option key={page.id} value={page.id}>{page.title}</option>)}</select></div><div className={styles.editorField}><span>Slug</span><output className={styles.editorFieldValue}>{slug}</output></div></div><div className={styles.editorSideCard}><div className={styles.aiHint}><span>✦</span><p className={styles.aiHintText}>Docent will index this page the moment you publish, so it can be cited in chat answers right away.</p></div></div></aside></form></main></>;
}
