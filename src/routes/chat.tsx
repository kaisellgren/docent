...
function ChatPage() {
  ...
  const visibleConversations = conversations.filter((conversation) =>
    conversation.title.toLowerCase().includes(conversationQuery.toLowerCase().trim()),
  )
        ...
        <main className={`${styles.shell} ${styles.chatWorkspace}`}>
          <aside className={styles.chatHistory}>
            ...
            <label className={styles.chatHistorySearch}>
              ...
              <input
                className={styles.chatHistorySearchInput}
              ...
              {conversationsLoading ? (
                ...
                visibleConversations.map((conversation) => (
                  <ConversationButton
                    ...
                    onClick={() => {
                      setQuestion('')
                      void router.navigate({ to: '/chat', search: { q: '', conversationId: conversation.id } })
                    }}
                    onDelete={() => {
                      void deleteSelectedConversation(conversation.id)
                    }}
                  />
                ))
              )}
          ...
          <section className={styles.chatMain}>
            <header className={styles.chatMainHead}>
              ...
              <span className={styles.chatStatus}>
                <span className={styles.chatStatusDot} /> Ready
              </span>
            </header>
            <div className={styles.chatMessages} ref={messagesRef}>
              {displayedMessages.length === 0 && !loading ? (
                <div className={styles.chatIntro}>
                  <div className={styles.chatIntroIcon}>
                    <Sparkles size={20} />
                  </div>
                  <h2 className={styles.chatIntroTitle}>What would you like to find?</h2>
                  <p className={styles.chatIntroText}>
                    Ask about a policy, project, decision, or owner. Docent will show the pages it used.
                  </p>
                  <div className={styles.chatSuggestions}>
                    {['What changed recently?', 'Who owns this project?', 'Summarize our on-call policy'].map(
                      (suggestion) => (
                        <button
              ...
              {loading && (
                <div className={styles.chatThinking} role="status" aria-live="polite">
                  <span className={styles.chatThinkingIcon}>
                    <Sparkles size={15} />
                  </span>
                  <span className={styles.chatThinkingDots}>
                    <i className={styles.chatThinkingDot} />
                    <i className={styles.chatThinkingDot} />
                    <i className={styles.chatThinkingDot} />
                  </span>
                  <span>Docent is weaving an answer…</span>
                </div>
              )}
            </div>
          ...
          <aside className={styles.chatReferences}>
            <div className={styles.chatReferencesHead}>
              <div>
                <span className={styles.chatKicker}>Grounding</span>
                <h2>References</h2>
              </div>
              <span>{references.length}</span>
            </div>
            {references.length === 0 ? (
              <div className={styles.chatReferenceEmpty}>
                <BookOpen size={18} />
                <p>Sources used by Docent will appear here.</p>
              </div>
            ) : (
              <div className={styles.chatReferenceList}>
                {references.map((reference) => (
                  <ReferenceCard
                    key={referenceId(reference.messageId, reference.citation)}
                    reference={reference}
...
function ConversationButton({
  ...
  return (
    <div className={styles.chatConversationRow}>
      <button
        type="button"
        className={active ? styles.chatConversationActive : styles.chatConversation}
        onClick={onClick}
      >
        <MessageSquare size={15} />
        <span>
          <strong className={styles.chatConversationTitle}>{conversation.title}</strong>
          <small className={styles.chatConversationMeta}>
            <Clock3 size={11} />
      ...
      <button
        type="button"
        className={styles.chatConversationDelete}
        onClick={onDelete}
...
function MessageBubble({
  ...
  onPreview: (file: { id: string; filename: string }) => void
}) {
          ...
          {message.citations.map((citation) => (
            <a
              ...
              onClick={(event) => {
                if (citation.fileId) {
                  event.preventDefault()
                  onPreview({ id: citation.fileId, filename: citation.title })
                }
              }}
            >
  ...
  return (
    <article id={referenceId(messageId, citation)} className={styles.chatReferenceCard}>
      ...
      {citation.fileId ? (
        <button
          type="button"
          className={styles.chatReferenceLinkButton}
          onClick={() => onPreview({ id: citation.fileId!, filename: citation.title })}
        >
          <strong>{citation.title}</strong>
        </button>
      ) : citation.slug ? (
        <a className={styles.chatReferenceLink} href={`/spaces/${citation.slug}`}>
          <strong>{citation.title}</strong>
        </a>
      ) : (
        <strong className={styles.chatReferenceLink}>{citation.title}</strong>
      )}
