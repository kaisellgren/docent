  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (problem)
          return new Response(problem, { status: 503, headers: { 'Content-Type': 'text/plain; charset=utf-8' } })
        const url = new URL(request.url)
        const error = url.searchParams.get('error')
        const code = url.searchParams.get('code')
        const state = url.searchParams.get('state')
        if (error || !code || !state)
          return new Response('Google sign-in was cancelled or could not be completed.', { status: 400 })
