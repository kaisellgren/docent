...
import { wikiRepository } from '@/server/dependencies'
...
const pageSchema = pageSummarySchema.extend({
  ...
  spaceSlug: z.string(),
  spaceName: z.string(),
  spaceIcon: z.enum(['book-open', 'code-2', 'compass', 'database', 'megaphone', 'palette', 'shield-check', 'users']),
...
const createPageSchema = pageInputSchema.extend({
  spaceId: z.string().uuid(),
  parentPageId: z.string().uuid().nullable(),
})
...
export const getRecentPages = createServerFn({ method: 'GET' }).handler(async () => {
  ...
  return wikiRepository.recentPages()
})

export const getSpaces = createServerFn({ method: 'GET' }).handler(async () => {
  const user = await requireSession()
  return wikiRepository.spaces(user.userId)
})
...
export const getSpace = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    const user = await requireSession()
    return wikiRepository.space(data.slug, user.userId)
  })
...
export const getSpacePages = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    await requireSession()
    return wikiRepository.pages(data.spaceId)
  })
...
export const createSpace = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const existing = await (
      await db()
    ).maybeOne(sql.type(z.object({ id: z.string().uuid() }))`SELECT id FROM wiki_space WHERE slug = ${slug}`)
      ...
      if (existing) {
        throw new Response(
          existing.deletedAt
            ...
            : 'A page with this title already exists. Choose a different title.',
          { status: 409 },
        )
      }
...
export const movePage = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    `)
    if (!page) throw new Response('Page not found', { status: 404 })
    if (data.destinationParentId === data.pageId)
      throw new Response('A page cannot be moved under itself.', { status: 400 })
    if (data.destinationParentId) {
      ...
      if (descendant.isDescendant)
        throw new Response('A page cannot be moved under one of its descendants.', { status: 400 })
    }
    await pool.query(
      sql.unsafe`UPDATE wiki_page SET parent_page_id = ${data.destinationParentId}, updated_at = now() WHERE id = ${data.pageId} AND deleted_at IS NULL`,
    )
    return { ok: true }
  })
...
export const updatePage = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const pool = await db()
    const result = await pool.transaction(async (transaction) => {
      const current = await transaction.one(sql.type(
        z.object({ id: z.string().uuid(), revisionNumber: z.number().int() }),
      )`
...
export const restorePageRevision = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const pool = await db()
    const result = await pool.transaction(async (transaction) => {
      const current = await transaction.maybeOne(sql.type(
        z.object({ id: z.string().uuid(), revisionNumber: z.number().int() }),
      )`
        SELECT p.id, r.revision_number AS "revisionNumber"
        FROM wiki_page p
...
export const deletePage = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    await (
      ...
      sql.unsafe`UPDATE wiki_page SET deleted_at = now(), updated_at = now() WHERE slug = ${data.slug} AND deleted_at IS NULL`,
    )
    return { ok: true }
  })
