  ...
  return (
    <header className={styles.pageViewNav}>
      <div className={`${styles.shell} ${styles.pageViewNavInner}`}>
        ...
        {viewer && <nav className={styles.pageViewCenterLinks} aria-label="Primary navigation">
          <Link to="/spaces">Spaces</Link>
          <Link to="/spaces/new" search={{ spaceId: createPageContext?.spaceId ?? "", parentPageId: createPageContext?.parentPageId ?? "" }}>Create page</Link>
          <Link to="/chat" search={{ q: "", conversationId: "" }}>Conversations</Link>
        </nav>}
        <div className={styles.pageViewNavRight}>
          {viewer && <Link className={styles.pageIconButton} to="/chat" search={{ q: "", conversationId: "" }} aria-label="Search conversations"><Search size={16} /></Link>}
          {viewer ? (
...
export function GoogleMark() {
  return <svg className={styles.googleMark} viewBox="0 0 24 24" aria-hidden="true"><path fill="#4285F4" d="M21.35 12.27c0-.72-.06-1.41-.18-2.07H12v3.92h5.24a4.48 4.48 0 0 1-1.94 2.94v2.44h3.14c1.84-1.69 2.91-4.18 2.91-7.23Z"/><path fill="#34A853" d="M12 21.5c2.63 0 4.84-.87 6.45-2.36l-3.14-2.44c-.87.58-1.98.92-3.31.92-2.54 0-4.69-1.72-5.46-4.03H3.3v2.52A9.74 9.74 0 0 0 12 21.5Z"/><path fill="#FBBC05" d="M6.54 13.59A5.86 5.86 0 0 1 6.23 12c0-.55.11-1.09.31-1.59V7.89H3.3A9.74 9.74 0 0 0 2.25 12c0 1.57.38 3.05 1.05 4.11l3.24-2.52Z"/><path fill="#EA4335" d="M12 6.38c1.43 0 2.71.49 3.72 1.45l2.79-2.79C16.84 3.47 14.63 2.5 12 2.5a9.74 9.74 0 0 0-8.7 5.39l3.24 2.52C7.31 8.1 9.46 6.38 12 6.38Z"/></svg>;
}
