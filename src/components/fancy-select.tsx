  ...
  return (
    ...
    >
      <Select.Trigger className={`${styles.fancySelectTrigger} ${className ?? ""}`}>
        ...
        <Select.Icon className={styles.fancySelectIcon}><ChevronDown size={15} /></Select.Icon>
      </Select.Trigger>
      <Select.Portal>
        <Select.Positioner className={styles.fancySelectPositioner} sideOffset={6}>
          <Select.Popup className={styles.fancySelectPopup}>
            <Select.List>
              {options.map((option) => (
                <Select.Item className={styles.fancySelectItem} key={option.value} value={option.value} label={option.label}>
                  <Select.ItemText>{option.label}</Select.ItemText>
                  <Select.ItemIndicator className={styles.fancySelectIndicator}><Check size={14} /></Select.ItemIndicator>
                </Select.Item>
              ))}
            </Select.List>
          </Select.Popup>
