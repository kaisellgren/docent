...
export function getDocentAgent(): Agent {
  ...
  const vertex = createVertex({
    project: configuration.GOOGLE_CLOUD_PROJECT,
    location: configuration.VERTEX_AI_LOCATION,
  })
  docentAgent = new Agent({
    ...
    instructions:
      'You are Docent, a precise internal knowledge assistant. Answer only from the supplied knowledge. Cite source numbers such as [1] when you rely on them. If evidence is insufficient, say so.',
  })
