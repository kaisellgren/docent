...
class DocentDev extends TerraformStack {
  constructor(scope: Construct, id: string) {
    ...
    for (const service of [
      ...
      enabledServices.set(
        service,
        new google.projectService.ProjectService(this, service.replaceAll('.', '-'), {
          service,
    ...
    const githubProvider = new google.iamWorkloadIdentityPoolProvider.IamWorkloadIdentityPoolProvider(
      this,
      'github-provider',
      {
    ...
    for (const role of [
      ...
      new google.projectIamMember.ProjectIamMember(
        this,
        `github-deployer-${role.replace('roles/', '').replaceAll('.', '-')}`,
        {
          project,
          role,
    ...
    const secretIds = [
      'docent-neon-url',
      'docent-google-client-id',
      'docent-google-client-secret',
      'docent-session-secret',
    ]
    const secrets = secretIds.map(
      (secretId) =>
        new google.secretManagerSecret.SecretManagerSecret(this, secretId, {
          secretId,
          replication: { auto: {} },
    ...
    new google.serviceAccountIamMember.ServiceAccountIamMember(this, 'app-task-identity', {
    ...
    if (process.env.DOCENT_DEPLOY_WEB === 'false') {
      ...
      new TerraformOutput(this, 'bootstrap_complete', {
        value: 'Set DOCENT_DEPLOY_WEB=true after pushing the image and adding secret versions.',
      })
      return
    }
        ...
        containers: [
          {
            ...
            env: [
              ...
              { name: 'DATABASE_URL', valueSource: { secretKeyRef: { secret: 'docent-neon-url', version: 'latest' } } },
              {
                ...
                valueSource: { secretKeyRef: { secret: 'docent-google-client-id', version: 'latest' } },
              },
              {
                name: 'GOOGLE_CLIENT_SECRET',
                valueSource: { secretKeyRef: { secret: 'docent-google-client-secret', version: 'latest' } },
              },
              { name: 'DOCENT_OAUTH_SECRET_ROLLOUT', value: oauthSecretRollout },
              {
                name: 'SESSION_SECRET',
                valueSource: { secretKeyRef: { secret: 'docent-session-secret', version: 'latest' } },
              },
